...
df_wq <- read_csv('Water_Quality/temp_wq_file.csv')

df_wq <- subset (df_wq, DS == 'EMP')
...
df_wq_seasonal <- df_wq %>%
  ...
  summarise_at(c('DON', 'DissNitrateNitrite', 'TotPhos', 'DissSilica'), mean, na.rm = TRUE)

df_wq_seasonal$DissNitrateNitrite <- log10(df_wq_seasonal$DissNitrateNitrite)
df_wq_seasonal$DON <- log10(df_wq_seasonal$DON)
df_wq_seasonal$TotPhos <- log10(df_wq_seasonal$TotPhos)
...
# Seasonal Averages for Entire Delta (TotPhos)
```{r}
print('TEST THINGS')
lm_seasonal <- lm(TotPhos ~ WY + Season, data = df_wq_seasonal)
...
# failed tests, add lags based on pacf
df_wq_seasonal$TotPhos_onelag <- lag(df_wq_seasonal$TotPhos, n = 1L)
df_wq_seasonal$TotPhos_twolag <- lag(df_wq_seasonal$TotPhos, n = 2L)
df_wq_seasonal$TotPhos_threelag <- lag(df_wq_seasonal$TotPhos, n = 3L)
df_wq_seasonal$TotPhos_fourlag <- lag(df_wq_seasonal$TotPhos, n = 4L)

lm_seasonal <- lm(TotPhos ~ WY + Season + TotPhos_onelag + TotPhos_twolag + TotPhos_threelag + TotPhos_fourlag, data = df_wq_seasonal)
...
# failed tests, add lags based on pacf
df_wq_seasonal$DissSilica_onelag <- lag(df_wq_seasonal$DissSilica, n = 1L)
df_wq_seasonal$DissSilica_twolag <- lag(df_wq_seasonal$DissSilica, n = 2L)
df_wq_seasonal$DissSilica_threelag <- lag(df_wq_seasonal$DissSilica, n = 3L)
df_wq_seasonal$DissSilica_fourlag <- lag(df_wq_seasonal$DissSilica, n = 4L)

lm_seasonal <- lm(DissSilica ~ WY + Season + DissSilica_onelag + DissSilica_twolag + DissSilica_threelag + DissSilica_fourlag, data = df_wq_seasonal)
...
df_wq_regional$TotPhos <- log10(df_wq_regional$TotPhos)
```
...
# Regional Averages per Year (TotPhos)
```{r}
print('TEST THINGS')
lm_regional <- lm(TotPhos ~ WY + SubRegion, data = df_wq_regional)
...
# failed tests, add lags based on pacf
df_wq_regional$TotPhos_onelag <- lag(df_wq_regional$TotPhos, n = 1L)
df_wq_regional$TotPhos_twolag <- lag(df_wq_regional$TotPhos, n = 2L)
df_wq_regional$TotPhos_threelag <- lag(df_wq_regional$TotPhos, n = 3L)
df_wq_regional$TotPhos_fourlag <- lag(df_wq_regional$TotPhos, n = 4L)

lm_regional <- lm(TotPhos ~ WY + SubRegion  + TotPhos_onelag + TotPhos_twolag + TotPhos_threelag + TotPhos_fourlag, data = df_wq_regional)
