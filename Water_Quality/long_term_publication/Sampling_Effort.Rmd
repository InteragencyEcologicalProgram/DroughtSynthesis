...
Hmmm, the problem seems to be coming from the one current EMP station (C3A) in the North region. Let's look at the data directly from the `discretewq` package (version 2.3.2) to see if other current EMP stations have secchi depth data during the same time period.

# Secchi Sampling Effort for current EMP stations

```{r plot secchi sampling effort all EMP, fig.width = 8.5, fig.height = 17}
# Pull out secchi depth data from EMP dataset within discretewq
EMP_secchi <- discretewq::EMP %>% 
  transmute(
    Station,
    Date = date(Date),
    Year = year(Date),
...
# Create vector of EMP stations with data collected since 2010 to only look at
  # recent or current stations
EMP_sta_rec <- EMP_secchi %>% 
  filter(Year >= 2010) %>% 
  distinct(Station) %>% 
  pull(Station)

# Plot sampling effort for secchi depth for recent or current EMP stations
EMP_secchi %>% 
  filter(Station %in% EMP_sta_rec) %>% 
  plot_samp_effort(grp_var = "Station", yr_var = "Year")
```

It looks like only station C3A has this issue of missing secchi depth data from 2017-2021.
