...
test <- read_csv(drought_abs_path(fp_abs_wq))
...
# clean wq file
df_wq$Date <- as.Date(df_wq$Date, '%m/%d/%Y', tz = 'UTC')
...
# add longterm col 
sf_wq_lt <- merge(sf_wq, df_regions[,c('Long_term','Season','SubRegion')], by = c('Season','SubRegion')) # add the region col as well
...
# add in the regions from rosie's file
```
