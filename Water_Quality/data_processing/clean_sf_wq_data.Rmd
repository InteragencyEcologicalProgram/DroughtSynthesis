...
```{r}
# define fps
fp_abs_wq_all <- 'Data/Component data sets/SF_water_quality/'
fp_abs_wq_old <- paste0(fp_abs_wq_all,'SanFranciscoBayWaterQualityData1969-2015v3.csv')
fp_abs_meta <- 'WQ_Team/metadata_wq_files/'
...
df_coords <- read_csv(drought_abs_path(paste0(fp_abs_meta,'drought_coords.csv')))
```
...
wq_coords <- left_join(df_coords, wq, by = c('Station'))

wq_coords <- add_season_col(wq_coords)

wq_coords <- wq_coords %>% rename(Region = Location)
```

```{r}
# convert wq to spdf
coords <- wq_coords[,c('Longitude', 'Latitude')]
data   <- wq_coords[,c(1,2,5:length(wq_coords))]
...
spdf_delta <- spTransform(spdf_delta, CRS('+proj=longlat +datum=WGS84 +no_defs'))
...
# add longterm col
sf_wq_lt <- merge(sf_wq, df_regions[,c('Long_term','SubRegion')], by = 'SubRegion')
...
write_csv(df_wq_lt, drought_abs_path('processed_data/SFBayWQData1969-2021.csv'))
```
